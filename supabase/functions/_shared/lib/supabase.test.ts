...
Deno.test("validatePostgrestSelectClause - accepts simple column list", () => {
  const result = validatePostgrestSelectClause("perfil_data, convenio_id");

  assertEquals(result.valid, true);
  assertEquals(result.error, undefined);
});

Deno.test("validatePostgrestSelectClause - accepts json accessor when complete", () => {
  const result = validatePostgrestSelectClause(
    "perfil_data->categorias_profesionales",
  );

  assertEquals(result.valid, true);
});

Deno.test("validatePostgrestSelectClause - rejects trailing dash", () => {
  const result = validatePostgrestSelectClause("perfil_data-");

  assertEquals(result.valid, false);
  assertEquals(result.error, 'select clause cannot end with "-"');
});

Deno.test("validatePostgrestSelectClause - rejects dangling json accessor", () => {
  const result = validatePostgrestSelectClause("perfil_data->");

  assertEquals(result.valid, false);
  assertEquals(result.error, "select clause has an incomplete JSON accessor");
});
